...
//        printf("min max OMP cost: %fms\n", t_bbox.toc());
    ...
    int64_t dx = static_cast<int64_t>((max_p[0] - min_p[0]) * inverse_leaf_size_[0])+1; //voxel索引最大差值
    ...
    min_b_[0] = static_cast<int> (floor (min_p[0] * inverse_leaf_size_[0])); //最小最大voxel索引
    ...
    div_b_ = max_b_ - min_b_ + Eigen::Vector4i::Ones (); // 三个维度上的 voxel 数量
    div_b_[3] = 0;
...
//    printf("bbox cost: %fms\n", t_bbox.toc());
    ...
    int centroid_size = 4; // 中心的维度
    if (downsample_all_data_) {
        centroid_size = boost::mpl::size<FieldList>::value; //所有field的中心维度
//        printf("downsample_all_data_\n");
    ...
    if (rgba_index >= 0)
    {
        ...
        centroid_size += 3; // centroid 维度 + 3 储存rgb
    }
...
//        PointCloud cloud_thread;
        std::vector<cloud_point_index_idx> index_vector; // voxel索引，点的数据指针
...
//#pragma omp parallel
            for (std::vector<int>::const_iterator it = indices_->begin(); it != indices_->end(); it += thread_id) {
                ...
                const uint8_t *pt_data = reinterpret_cast<const uint8_t *> (&input_->points[*it]); //指向点数据的指针
...
//            for (std::vector<int>::const_iterator it = indices_->begin(); it != indices_->end(); ++it) {
            for (size_t i = 0; i < indices_->size(); ++i) {
                ...
                int ijk0 = static_cast<int> (floor(point.x * inverse_leaf_size_[0]) -
                                             static_cast<float> (min_b_[0])); //第一个维度索引
                int ijk1 = static_cast<int> (floor(point.y * inverse_leaf_size_[1]) -
                                             static_cast<float> (min_b_[1])); //第二个维度索引
                int ijk2 = static_cast<int> (floor(point.z * inverse_leaf_size_[2]) -
                                             static_cast<float> (min_b_[2])); //第三个维度索引

                // Compute the centroid leaf index
                int idx = ijk0 * divb_mul_[0] + ijk1 * divb_mul_[1] + ijk2 * divb_mul_[2]; //点所在的voxel索引
                /// !!! so slow  !!!
                index_vector.emplace_back(
                        cloud_point_index_idx(static_cast<unsigned int> (idx), point_id)); //voxel索引，指向数据的指针
            }
        }
...
//        TicToc t_sort;
        ...
        std::sort(index_vector.begin(), index_vector.end(), std::less<cloud_point_index_idx>()); //按voxel索引从小到大排序
...
//    TicToc t_valid_voxel;
        ...
        // Worst case size
        first_and_last_indices_vector.reserve(index_vector.size()); //每一个voxel的第一个点和最后一个点在排序后voxel向量中的索引
        //双指针查找同一voxel内的起点和终点
        while (index < index_vector.size()) {
            ...
            while (i < index_vector.size() && index_vector[i].idx == index_vector[index].idx)
                ++i;
            //todo
            if (i - index >= min_points_per_voxel_) //珊格内点数足够多
            {
                ++total; //有效voxel + 1
...
//#pragma omp parallel num_threads(threads_)
//    {
        Eigen::VectorXf centroid = Eigen::VectorXf::Zero (centroid_size); // 中心向量，包括多有field的维度
        Eigen::VectorXf temporary = Eigen::VectorXf::Zero (centroid_size); //临时变量
//        std::unordered_map<int, Voxel> voxels;
//#pragma omp for
        for (unsigned int cp = 0; cp < first_and_last_indices_vector.size(); ++cp) // 遍历所有voxel
        {
            // calculate centroid - sum values from all input points, that have the same idx value in index_vector array
            unsigned int first_index = first_and_last_indices_vector[cp].first; //第一个点在有效点云中的索引
            unsigned int last_index = first_and_last_indices_vector[cp].second; //最后一个点在有效点云中的索引
//            int voxel_id = index_vector[first_index].idx;
            if (!downsample_all_data_) {
                // index_vector[first_index].cloud_point_index 第一个点在原始点云中的索引
                // 记录第一个点的xyz
            ...
            for (unsigned int i = first_index + 1; i < last_index; ++i)  //累加voxel内所有点的xyz和其他数据
            {
                if (!downsample_all_data_) {
                    ...
                    pcl::for_each_type<FieldList>(
                            NdCopyPointEigenFunctor<PointT>(input_->points[index_vector[i].cloud_point_index],
                                                            temporary));
                    centroid += temporary; //累计当前点的所有数据到centroid变量
                }
            }

            // index is centroid final position in resulting PointCloud
            if (save_leaf_layout_)
//            leaf_layout_[index_vector[first_index].idx] = index; //layout[voxel id] = 输出的voxel索引
                leaf_layout_[index_vector[first_index].idx] = cp; //layout[voxel id] = 输出的voxel索引

                //todo 记录点数
            centroid /= static_cast<float> (last_index - first_index); //计算中心
